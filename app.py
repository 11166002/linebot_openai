...
def callback():
    try:
        ...
        if body is None:
            return "Error: No JSON data received", 400  # 無 JSON 資料時返回錯誤
        ...
        return jsonify({"error": str(e)}), 500  # 返回錯誤資訊給客戶端
...
def reply_text(reply_token, text):
    ...
    try:
        ...
        response.raise_for_status()  # 如果回應不是 200，會觸發例外
    except requests.exceptions.HTTPError as errh:
        print(f"HTTP error occurred: {errh}")  # HTTP 錯誤
    except requests.exceptions.ConnectionError as errc:
        print(f"Error connecting: {errc}")  # 連線錯誤
    except requests.exceptions.Timeout as errt:
        print(f"Timeout error: {errt}")  # 超時錯誤
    except requests.exceptions.RequestException as err:
        print(f"Other error occurred: {err}")  # 其他錯誤
...
def maze_game(user, message):
    ...
    if player.get("quiz"):
        ...
        if message in choice_map and choice_map[message] == answer:
            ...
            return {"map": render_map(player["pos"]), "message": f"❓ 挑戰：「{kana}」的羅馬拼音是？請從下列選項選擇：\n{options_text}"}
